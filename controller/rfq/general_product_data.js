function rfq_product_linesValidation(req, res, callback){
	...
	if(typeof req.header("authentication_token")=="undefined" || req.header("authentication_token")==""){
		...
		console.log("SELECT `id`,`authentication_token` FROM `organization_users` WHERE authentication_token='"+req.header("authentication_token")+"' AND id='"+req.params.user_id+"'");
  	  ...
  	  connection.query("SELECT `id`,`authentication_token` FROM `organization_users` WHERE authentication_token='"+token+"' AND id="+req.body.user_id, function(err, organization_users) {
			if(err){
				...
				if (organization_users.length>0) {
						...
						if(err){
							...
							connection.query(query, function(err, tendering_teams_members) {
								if(err){
									...
									res.json({"statusCode": 200, "success":"true", "message":"", "rfq_id":req.body.rfq_id, "selected_rfq": rfq});
								}
							});
						}
					});
					...
					res.json({"statusCode": 404, "success": "false", "message": "Authentication token not valid or user id not valid"});
				}
			}
		});
  	}
  		...
  		// 	res.json({"statusCode": 422, "success": "false", "message": "product_line_id not found"});
  		// }
  		// else if(typeof req.body.product_lines_id==="undefined"){
  		// 	res.json({"statusCode": 422, "success": "false", "message": "parameter product_line_id not defined"});
  		// }
  		// else if(req.body.tendering_teams_teams_members_id===""){
  		// 	res.json({"statusCode": 422, "success": "false", "message": "tendering_teams_members_id not found"});
  		// }
  		// else if(typeof req.body.tendering_teams_teams_members_id==="undefined"){
  		// 	res.json({"statusCode": 422, "success": "false", "message": "parameter tendering_teams_members_id not defined"});
  		// }
  		// else if(req.body.rfq_id===""){
  		// 	res.json({"statusCode": 422, "success": "false", "message": "rfq_id not found"});
  		// }
  		// else{
  		// 	res.json({"statusCode": 422, "success": "false", "message": "parameter rfq_id not defined"});
  		// }
  	}
};
