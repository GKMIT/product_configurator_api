				...
				if(err){
					...
					connection.query("SELECT FROM `rfq_lines_technical_specs` `rlts` LEFT JOIN `product_properties` `pp` ON `rlts`.`product_properties_id`=`pp`.id WHERE `rfq_lines_id`='"+rfq_lines[0].id+"'", function(err, rfq_lines_technical_specs) {
						if(err){
							...
							connection.query("SELECT * FROM `product_designs` LEFT JOIN `product_designs_costs` ON `product_designs`.id=`product_designs_costs`.`product_designs_id` WHERE `product_designs_costs`.`quarter`='"+quarter+"'", function(err, product_design_detail) {
								if(err){
									console.log(err);
										res.json({"statusCode": 500, "success":"false", "message": "internal error"});
								}
